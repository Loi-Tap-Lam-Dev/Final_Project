...
void outInfor(School_Year* yearHead)
{
    ...
    OutData<<"Year"<<endl;
    for(School_Year*yearCur=yearHead;yearCur->yNext!=nullptr;yearCur=yearCur->yNext)
    {
    ...
    for(School_Year::Year_Class*year_Class_Cur=yearHead->yearCLassHead;year_Class_Cur->clNext!=nullptr;year_Class_Cur=year_Class_Cur->clNext)
    {
        ...
        for(School_Year::Year_Class::SV_List*year_Class_Sv_Cur=year_Class_Cur->yearClassSV_ListHead;year_Class_Sv_Cur->svNext!=nullptr;year_Class_Sv_Cur=year_Class_Sv_Cur->svNext)
        {
    ...
    for(School_Year::Semester*year_Semester_Cur=yearHead->yearSemesterHead;year_Semester_Cur->sNext!=nullptr;year_Semester_Cur=year_Semester_Cur->sNext)
    {
        string tmp=""",""";
        OutData<<"Term "+tmp+"Start Date"+tmp+"End Date"<<endl;
        OutData<<to_string(year_Semester_Cur->Term)+tmp+to_string(year_Semester_Cur->start_Date)+tmp+to_string(year_Semester_Cur->end_Date)<<endl;
        OutData<<"Sv Semester Mark "<<endl;
        OutData<<"No "+tmp+"Id "+tmp+"Credit "+tmp+"GPA "+tmp+"Average Mark "<<endl;
        string tmp=""",""";
        for (School_Year::Semester::Student_listMark* year_Semester_SvListCur = year_Semester_Cur -> yearSemesterSv_ListHead; year_Semester_SvListCur -> svNext != nullptr;year_Semester_SvListCur = year_Semester_SvListCur -> svNext )
        {
        ...
        for (School_Year::Semester::Subject* year_Semester_Subject_Cur = year_Semester_Cur -> yearSemesterSubjectHead;year_Semester_Subject_Cur -> subNext != nullptr; year_Semester_Subject_Cur = year_Semester_Subject_Cur -> subNext)
        {
            OutData<<year_Semester_Subject_Cur->name_Subject+tmp+year_Semester_Subject_Cur->id_Subject+tmp+year_Semester_Subject_Cur->startDate+tmp+year_Semester_Subject_Cur->endDate+tmp+year_Semester_Subject_Cur->teacher_Name+tmp+year_Semester_Subject_Cur->day_Of_Session_1+tmp+year_Semester_Subject_Cur->day_Of_Session_2+tmp+year_Semester_Subject_Cur->at_Time_1+tmp+year_Semester_Subject_Cur->at_Time_2+tmp+year_Semester_Subject_Cur->maximumRegrister+tmp+year_Semester_Subject_Cur->number_Of_Credit<<endl;
        }
    }
    OutData<<endl;
    OutData.close();
