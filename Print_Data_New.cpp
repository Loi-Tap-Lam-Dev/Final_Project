...
void PrintData(School_Year*yearHead)
{
    ...
    for(School_Year*year_cur=yearHead;year_cur!=nullptr;year_cur=year_cur->Next)
    {
        PrintYearSemester<<"Semester For Year"+tmp+to_string(year_cur->year)<<endl;
        ...
        {
            PrintYearSemester<<"Semester info"+tmp+"Term"+tmp+" Start Date"+tmp+" End Date"<<endl;
            PrintYearSemester<<to_string(year_semester_cur->Term)+tmp+year_semester_cur->start_Date+tmp+year_semester_cur->end_Date<<endl;
            PrintYearSemester<<"Sv_Semester_Mark"+tmp+"No"+tmp+"Id"+tmp+"Fullname"+tmp+"Total Credit"+tmp+"GPA"+tmp+"Average Mark"<<endl;
            ...
            {
                PrintYearSemester<<to_string(year_semester_stu_cur->no)+tmp+to_string(year_semester_stu_cur->idStudent)+tmp+year_semester_stu_cur->fullName+tmp+to_string(year_semester_stu_cur->totalCredit)+tmp+to_string(year_semester_stu_cur->GPA)+tmp+to_string(year_semester_stu_cur->averageMark)<<endl;
            }
            PrintYearSemester<<"Subject"+tmp+"NameSubject"+tmp+" IDSubject"+tmp+" StartDate"+tmp+" EndDate"+tmp+" TeacherName"+tmp+" Day_of_session_1"+tmp+" Time_Day_1"+tmp+" Day_of_session_2"+tmp+" Time_Day_2"+tmp+" Number_Credit"+tmp+"MaxRegister"<<endl;
            {
